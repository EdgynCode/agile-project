...
if (!empty($_POST['title'])) {
    ...
    if (isset($_FILES["thumbnail_1"])) {
        ...
        $file = $_FILES["thumbnail_1"];
        
        // Check if the file upload was successful
        if ($file["error"] == 0) {
            // Move the uploaded file to your desired directory and get its new path
            $target_dir = "uploads/";
            $target_file = $target_dir . basename($file["name"]);
            if (move_uploaded_file($file["tmp_name"], $target_file)) {
                // Assign the file's path to $thumbnail_1
                $thumbnail_1 = $target_file;
            } else {
                echo "There was an error uploading the file.";
                die;
            }
        } else {
            echo "There was an error with the file upload: " . $file["error"];
            die;
        }
    } else {
        // If no new file, keep the old value from the database
    ...
    for ($i = 1; $i <= 5; $i++) {
        if (isset($_FILES['thumbnail_' . $i]) && $_FILES['thumbnail_' . $i]['error'] == 0) {
            // Handle the uploaded file...
            $file = $_FILES['thumbnail_' . $i];
            ...
            // Assign the file's path to the corresponding variable
            ${'thumbnail_' . $i} = $target_file;
        } else {
            // If no new file, keep the old value from the database
            ${'thumbnail_' . $i} = ${'thumbnail_in_database_' . $i};
        }
    }
...
<div class="d-flex flex-column flex-lg-row h-lg-full bg-surface-secondary">
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="panel-body">
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <button id="add-more" type="button" class="btn btn-primary btn-sm">Thêm 1 Ảnh</button> 
                                    <button id="del-down" type="button" class="btn btn-primary btn-sm">Xóa ảnh phụ</button> 
                                <div class="form-group">
    ...
    <script>
        $(document).on('change', 'input[id^="thumbnail_"]', function() {
        var file = this.files[0];
        if (file) {
            var reader = new FileReader();
            reader.onload = function(e) {
                // Get the id of the input field, replace 'thumbnail_' with 'img_thumbnail_' to get the id of the image element
                var id = $(this).attr('id');
                var imgId = id.replace('thumbnail_', 'img_thumbnail_');
...
<script type="text/javascript">
        ...
        $('#add-more').on('click', function() {
            if (thumbnailIndex <= 5) {
                $('#thumbnails-container').append(`
                    <div class="form-group">
                        <label for="name">Ảnh phụ:</label>
                        <input type="file" class="form-control-file" id="thumbnail_${thumbnailIndex}" name="thumbnail_${thumbnailIndex}">
                        <img style="max-width: 200px" id="img_thumbnail_${thumbnailIndex}">
                    </div>
                `);
    ...
    <script>
    $(document).ready(function() {
        $('#del-down').click(function() {
            // Remove thumbnail elements
            $('#thumbnails-container').empty();

            // Clear images from thumbnails
            for (var i = 1; i <= 5; i++) {
                $('#thumbnail_' + i).val('');
                $('#img_thumbnail_' + i).attr('src', '');
            }
        });
    });
    </script>
